(function (global, factory) {
  ...
  function attach(element, editor) {
    ...
    function pointerDownHandler(evt) {
      ...
      //   }
      // } else 
      if (evt.button !== 2 && evt.buttons !== 2 && pointerDownOnEditor) {
        // Ignore right click
        if (!hideMenu(evt) && !hideCandidates(evt)) {
          this.activePointerId = evt.pointerId || 1;
  ...
  function manageRecognizedModel(editor, model) {
    ...
    if (modelRef.exports && editor.configuration.recognitionParams.recognitionHandlers) {
      var handlers = editor.configuration.recognitionParams.recognitionHandlers;
      for (var i = 0, ln = handlers.length; i < ln; i++) {
        var handler = handlers[i];
        handler.call({}, 
          [
            JSON.parse(modelRef.exports[Constants.Exports.JIIX])
          ]
        );
      }
    }
